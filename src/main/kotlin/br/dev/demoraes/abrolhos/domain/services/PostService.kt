...
// TODO(Remove double bangs)
    ...
    fun createDraft(
        ...
        val category =
            categoryRepository.findByName(categoryVO)
                ?: throw NoSuchElementException("Category '${command.categoryName}' not found.")
    ...
    fun archivePost(postId: String): Post {
        ...
        check(post.status != PostStatus.ARCHIVED) {
    ...
    fun updatePost(
        ...
        val tags =
            command.tagNames?.map { TagName(it) }?.map { tagName ->
                tagRepository.findByName(tagName) ?: throw NoSuchElementException("Tag '${tagName.value}' not found.")
            }?.toSet() ?: existing.tags
