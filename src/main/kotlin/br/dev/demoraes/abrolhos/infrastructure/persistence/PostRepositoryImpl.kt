...
class PostRepositoryImpl(
    ...
    override fun findPublished(): Set<Post> {
        return postRepositoryPostgresql.findByStatus(PostStatus.PUBLISHED).map { it.toDomain() }.toSet()
    }
...
private fun CategoryEntity.toDomain(): Category {
    val created =
        this.createdAt
            ?: throw IllegalStateException("CategoryEntity with id ${this.id} is missing a createdAt timestamp.")
    val updated =
        this.updatedAt
            ?: throw IllegalStateException("CategoryEntity with id ${this.id} is missing an updatedAt timestamp.")
    return Category(
...
private fun TagEntity.toDomain(): Tag {
    val created =
        this.createdAt
            ?: throw IllegalStateException("TagEntity with id ${this.id} is missing a createdAt timestamp.")
    val updated =
        this.updatedAt
            ?: throw IllegalStateException("TagEntity with id ${this.id} is missing an updatedAt timestamp.")
    return Tag(
        id = ULID.parseULID(this.id),
