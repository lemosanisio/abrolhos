...
class TagService(
    ...
    fun create(command: CreateTagCommand): Tag {
        ...
        check(tagRepository.findByName(name) == null) {
            "Tag with name ${command.name} already exists"
        }
        check(tagRepository.findBySlug(slug) == null) {
            "Tag with slug ${slug.value} already exists"
        }
    ...
    fun update(command: UpdateTagCommand): Tag {
        ...
        tagRepository.findByName(name)?.let {
            check(it.id == tag.id) { "Tag with name ${command.name} already exists" }
        }
        tagRepository.findBySlug(slug)?.let {
            check(it.id == tag.id) { "Tag with slug ${slug.value} already exists" }
        }
